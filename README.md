...
Внутренняя версия сурового постоянно работающего multiplayer-survival на Unity
`6000.5.8f1`. Windows-клиент входит через Steam, подключается к выделенному
Linux-серверу и получает детерминированный процедурный мир 2×2 км. Сервер
авторитетно симулирует организм, бой, NPC, погоду, постройки и постоянные тела;
...
- ASP.NET Core 10 профильный сервис, PostgreSQL 18, EF migration, автоматическое
  создание мира/профиля и атомарные снимки персонажей каждые 5 секунд;
- физиология без HP, травмы и последовательная медицина, потребности, сон,
  развитие 12 характеристик и навыков, ограниченные врождённые черты;
- физические карты и память персонажа, еда, жидкости, одежда, загрязнение,
  строительство, огонь, канализация, колодцы и детерминированная погода;
- постоянные NPC, восемь работ, отношения и договоры, PvP, плен, тела,
  разложение, наследники и посмертная передача наследника;
...
Основные команды в мире: `Tab` — инвентарь, `Q` — ремесло, `M` — физическая
карта, `H` — тело, `J` — развитие, `Z` — сон, `E` — контекстное действие.
ЛКМ выполняет лёгкий удар, удержание ЛКМ — тяжёлый, ПКМ — блок,
`Alt` + направление — уклонение.
