...
игроков и из каталога `Deploy` выполните:
...
dump в `Deploy/release-backups`, проверяет его через `pg_restore --list` и только
после этого одной транзакцией очищает мир, аккаунты, персонажей и имущество.
